    ...
    <div class="w-full" @mousedown.left="onMouseDown">
      <div class="w-full horizontal overflow-x-auto overflow-y-hidden pb-8 -mb-8" ref="horizontal" @scroll="onScroll">
        <div class="w-637.5 flex-shrink-0 overflow-hidden">
          <div class="w-660 grid grid-cols-50 -ml-11.5">
            <div/>
          ...
          <div class="w-647 h-20 relative -ml-5">
            <line-chart class="!w-647 !h-20 absolute inset-0 z-10" :min="Math.min(...Object.values(twoDayTemp)) - 3" :max="Math.max(...Object.values(twoDayTemp)) + 2" :data="twoDayTemp" :dataset="{ borderColor: '#0ea5e9' }" :library="chartConfig"/>
            <line-chart class="!w-647 !h-20 absolute inset-0 z-5 opacity-20" :min="Math.min(...Object.values(twoDayTemp)) - 3" :max="Math.max(...Object.values(twoDayTemp)) + 2" :data="twoDayTemp" :dataset="{ borderColor: 'rgba(0, 0, 0, 0)', backgroundColor: '#0ea5e9', fill: true }" :library="chartConfig"/>
            <div class="w-647 h-20 absolute inset-0 z-7" :style="`background: linear-gradient(180deg, rgba(${getBgColor((current.weather && inited) ? current.weather.class : 'sunny', currentSpan)}, 0) 60%, rgba(${getBgColor((current.weather && inited) ? current.weather.class : 'sunny', currentSpan)}, 1) 100%)`"/>
          </div>
          <div class="w-660 grid grid-cols-50 -ml-11.5 mt-0.5">
            <div/>
